...
function newamp1_fbf(samname, f=10)
  ...
  do
    ...
    b = regress(rate_K_vec_no_mean',rate_K_sample_freqs_kHz')
    n = rate_K_sample_freqs_kHz*b;
    rate_K_vec_no_mean_r = rate_K_vec - n - mean_f;
    %rate_K_vec_no_mean_r = rate_K_vec_r - mean(rate_K_vec_r);
    b2 = regress(rate_K_vec_no_mean_r',rate_K_sample_freqs_kHz')
    %rate_K_vec_no_mean = rate_K_vec_no_mean_r

    if quant_en == 2
      [res rate_K_vec_no_mean_ ind] = mbest(train_120_vq, rate_K_vec_no_mean_r, m);
    else
      rate_K_vec_no_mean_ = rate_K_vec_no_mean_r;
    end
    ...
    plot(rate_K_sample_freqs_kHz*1000, rate_K_vec_no_mean_r, ';rate K mel no mean desloped;go-');
    if quant_en == 2
